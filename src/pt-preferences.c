            ...
            gpointer   user_data)
{
	...
	g_signal_handlers_block_by_func (widget, config_activated, dlg);
}
              ...
              gpointer   user_data)
{
	...
	g_signal_handlers_unblock_by_func (widget, config_activated, dlg);
}
                  ...
                  gpointer    user_data)
{
	...
	GtkContainer *box = GTK_CONTAINER (dlg->priv->asr_list);
	PtConfig *config;
	GFile    *file;
	gchar    *path;
	...
	gtk_container_foreach (box, block_rows, dlg);
	...
	gtk_container_foreach (box, unblock_rows, dlg);
...
static void
asr_setup_config_box (PtPreferencesDialog *dlg)
{
	GError   *error = NULL;
	PtConfig *config;
	gchar    *path;
	...
	int       num_valid = 0;
	int       num_loadable = 0;
	GtkContainer *asr_list = GTK_CONTAINER (dlg->priv->asr_list);

	active_asr = g_settings_get_string (dlg->priv->editor, "asr-config");

	files = g_file_enumerate_children (dlg->priv->config_folder,
	...
	while (!error) {
		...
		if (g_str_has_suffix (path, ".asr")) {
			...
			if (pt_config_is_valid (config)) {
				num_valid++;
				if (pt_player_config_is_loadable (dlg->priv->player, config)) {
					num_loadable++;
					...
					g_signal_connect (row, "notify::active", G_CALLBACK (config_activated), dlg);
				}
			} else {
	...
	if (error || (num_valid > 0 && num_loadable == 0)) {
		gtk_widget_hide (dlg->priv->asr_initial_box);
		gtk_widget_hide (dlg->priv->asr_ready_box);
		gtk_widget_show (dlg->priv->asr_error_box);
		if (error) {
			gtk_label_set_text (GTK_LABEL (dlg->priv->asr_error_label),
		...
		asr_list_changed_cb (asr_list, NULL, dlg);
	}
}
                         ...
                         gpointer       user_data)
{
	...
	if (copy_asr_configs_finish (source_object, res, &error)) {
		g_signal_handlers_block_by_func (dlg->priv->asr_list,
		                                 asr_list_changed_cb, dlg);
		asr_setup_config_box (dlg);
		g_signal_handlers_unblock_by_func (dlg->priv->asr_list,
		                                   asr_list_changed_cb, dlg);
	} else {
		gtk_widget_hide (dlg->priv->asr_initial_box);
		gtk_widget_hide (dlg->priv->asr_ready_box);
...
{
	PtPreferencesDialog *dlg = PT_PREFERENCES_DIALOG (source_object);
	gchar    *basename;
	...
	dest_folder_path = g_file_get_path (dlg->priv->config_folder);
...
{
	GTask *task;
	task = g_task_new (dlg,  /* source object */
	                   NULL, /* cancellable   */
                    ...
                    gpointer      user_data)
{
	...
	if (success || (!success && g_error_matches (error, G_IO_ERROR,
	                                             G_IO_ERROR_EXISTS))) {
		gtk_widget_hide (dlg->priv->asr_error_box);
		asr_setup_config_box (dlg);
		g_signal_connect (dlg->priv->asr_list, "add",
		                  G_CALLBACK (asr_list_changed_cb), dlg);
		g_signal_connect (dlg->priv->asr_list, "remove",
		                  G_CALLBACK (asr_list_changed_cb), dlg);
	} else {
		gtk_widget_hide (dlg->priv->asr_initial_box);
		gtk_widget_hide (dlg->priv->asr_ready_box);
...
pt_preferences_dialog_init (PtPreferencesDialog *dlg)
{
	...
	path = g_build_path (G_DIR_SEPARATOR_S,
	                     ...
	                     PACKAGE, NULL);
	dlg->priv->config_folder = g_file_new_for_path (path);
	g_free (path);

	/* init is continued in make_config_dir_cb */
	g_file_make_directory_async (dlg->priv->config_folder,
	                             G_PRIORITY_DEFAULT,
	                             NULL, /* cancellable */
	                             make_config_dir_cb,
	                             dlg);
...
pt_preferences_dialog_dispose (GObject *object)
{
	...
	g_clear_object (&dlg->priv->config_folder);
...
pt_preferences_dialog_class_init (PtPreferencesDialogClass *klass)
{
	...
	gtk_widget_class_bind_template_child_private (widget_class, PtPreferencesDialog, asr_error_box);
	...
	gtk_widget_class_bind_template_child_private (widget_class, PtPreferencesDialog, asr_error_icon);
	gtk_widget_class_bind_template_child_private (widget_class, PtPreferencesDialog, asr_error_label);
